...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  $login_permission  = trim($_POST['login_permission']);
  ...
  /**********************************************************/
	...
	if($objMemborNumber->checkMemberNo()){
  ...
  if(!$vResult){
    ...
    if($objChurch->actMember($mode)){
      ...
      if($mode == "U"){
        $objCommon->setMessage("Staff member data updated successfully.", 'Info');
      } else {
        $objCommon->setMessage("Staff member data added successfully.",'Info');
      }

      $link = Route::_('show=staffmembering');
      redirect($link);
    }
  }
